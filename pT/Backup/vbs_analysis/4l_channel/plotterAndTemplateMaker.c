...
void plotterAndTemplateMaker(int year = 2018, int useMCatNLO = 1, int enriched = 0){
	...
	string namegif[vars] = {"Dbkgkin","m4l","mjj","detajj","ptj","etaj","Nvtx"};
        int bins[vars] = {20,30,30,20,40,30,35};
        float xmin[vars] = {0.,0.,100.,0.,0.,-5.,0.};
	float xmax[vars] = {1.,1500.,1300.,8.,400.,5.,70.};	
	...
	for (int it=0; it < 5; it++) {
	  ...
	  temp_1d_4e[it] = new TH1F("temp_1d_4e","",nBinsTempl,0.,1500.);
	  temp_1d_4mu[it] = new TH1F("temp_1d_4mu","",nBinsTempl,0.,1500.);
	  temp_1d_2e2mu[it] = new TH1F("temp_1d_2e2mu","",nBinsTempl,0.,1500.);
	}
	...
	for(int is = 0; is < nSamp-1; is++){
	  ...
	  for(int i=0;i<enne;i++){
		...
		tnew[0]->Fill();
		// if(chan==1) { 
		temp_zz_4mu[0]->Fill(ZZMass,dbkg_kin,weight); temp_1d_4mu[0]->Fill(Z1Pt,weight);  
		// else if(chan==2) { 
		temp_zz_4e[0]->Fill(ZZMass,dbkg_kin,weight); temp_1d_4e[0]->Fill(Z1Pt,weight); 
	        // else { 
		temp_zz_2e2mu[0]->Fill(ZZMass,dbkg_kin,weight); temp_1d_2e2mu[0]->Fill(Z1Pt,weight); 
		...
		tnew[j]->Fill();
		temp_zz_4mu[j]->Fill(ZZMass,dbkg_kin,weight);  temp_1d_4mu[j]->Fill(Z1Pt,weight);
		temp_zz_4e[j]->Fill(ZZMass,dbkg_kin,weight);  temp_1d_4e[j]->Fill(Z1Pt,weight);
	        temp_zz_2e2mu[j]->Fill(ZZMass,dbkg_kin,weight);  temp_1d_2e2mu[j]->Fill(Z1Pt,weight);
	      } else if (j==4) {
		tnew[0]->Fill();     tnew[2]->Fill();
		temp_zz_4mu[0]->Fill(ZZMass,dbkg_kin,weight);  temp_1d_4mu[0]->Fill(Z1Pt,weight);
		temp_zz_4e[0]->Fill(ZZMass,dbkg_kin,weight);  temp_1d_4e[0]->Fill(Z1Pt,weight);
	        temp_zz_2e2mu[0]->Fill(ZZMass,dbkg_kin,weight);  temp_1d_2e2mu[0]->Fill(Z1Pt,weight);
		if(chan==1) { temp_zz_4mu[2]->Fill(ZZMass,dbkg_kin,weight);  temp_1d_4mu[2]->Fill(Z1Pt,weight); }
		else if(chan==2) { temp_zz_4e[2]->Fill(ZZMass,dbkg_kin,weight); temp_1d_4e[2]->Fill(Z1Pt,weight); } 
	        else { temp_zz_2e2mu[2]->Fill(ZZMass,dbkg_kin,weight);  temp_1d_2e2mu[2]->Fill(Z1Pt,weight); }
	      } else if (j>9) {
		tnew[4]->Fill();
		temp_zz_4mu[4]->Fill(ZZMass,dbkg_kin,weight);  temp_1d_4mu[4]->Fill(Z1Pt,weight);
		temp_zz_4e[4]->Fill(ZZMass,dbkg_kin,weight);  temp_1d_4e[4]->Fill(Z1Pt,weight);
	        temp_zz_2e2mu[4]->Fill(ZZMass,dbkg_kin,weight);  temp_1d_2e2mu[4]->Fill(Z1Pt,weight);
	      } else {
		tnew[j]->Fill();
		if(chan==1) { temp_zz_4mu[j]->Fill(ZZMass,dbkg_kin,weight);  temp_1d_4mu[j]->Fill(Z1Pt,weight); }
		else if(chan==2) { temp_zz_4e[j]->Fill(ZZMass,dbkg_kin,weight); temp_1d_4e[j]->Fill(Z1Pt,weight); } 
	        else { temp_zz_2e2mu[j]->Fill(ZZMass,dbkg_kin,weight);  temp_1d_2e2mu[j]->Fill(Z1Pt,weight); }
	      }
