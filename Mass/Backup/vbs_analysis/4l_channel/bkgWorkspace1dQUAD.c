...
void dosomething(TString chan="2e2mu", int year=2016, int enriched = 0){
	...
	if (enriched == 5) theExtra = "_superVBSenrMjj";
        ...
        string namesamp[samples] = {"quad_vbs","zjet","qqzz","ggzz","ttzwzz"}; 
	...
	for (int is = 0; is < samples; is++) {
	  ...
	  if(is==0) sprintf(filename,"quadratic_1");
	...
	fwork= new TFile("workspace/quad_1D_vbs4lSyst_"+chan+Form("1S_%d.input_func%s.root",year,theExtra.c_str()),"recreate");
	fwork->cd();
	for (int is = 0; is < samples; is++) {
	  if (is==1) continue;    // zx later!
...
void bkgWorkspace1dQUAD(TString chan, int year, int isVBSenriched){
