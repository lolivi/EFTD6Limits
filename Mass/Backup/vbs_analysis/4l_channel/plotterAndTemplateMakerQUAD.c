...
void plotterAndTemplateMakerQUAD(int year = 2018, int useMCatNLO = 1, int enriched = 0){
	...
	for (int it=0; it < 5; it++) {
	  ...
	  if (it==2) sprintf(filename,"template/root_output_files/quad_vbs_Moriond_%d%s.root",year,theExtra.c_str()); 
	...
	for(int is = 0; is < nSamp-1; is++){
	  ...
	  for(int i=0;i<enne;i++){
		...
		tnew[0]->Fill();
		// if(chan==1) { 
		temp_1d_4mu[0]->Fill(ZZMass,weight);  
		// else if(chan==2) { 
		temp_1d_4e[0]->Fill(ZZMass,weight); 
	        // else { 
		temp_1d_2e2mu[0]->Fill(ZZMass,weight); 
		...
		tnew[j]->Fill();
		temp_1d_4mu[j]->Fill(ZZMass,weight);
		temp_1d_4e[j]->Fill(ZZMass,weight);
	        temp_1d_2e2mu[j]->Fill(ZZMass,weight);
	      } else if (j==4) {
		tnew[0]->Fill();     tnew[2]->Fill();
		temp_1d_4mu[0]->Fill(ZZMass,weight);
		temp_1d_4e[0]->Fill(ZZMass,weight);
	       	temp_1d_2e2mu[0]->Fill(ZZMass,weight);
		if(chan==1) {temp_1d_4mu[2]->Fill(ZZMass,weight); }
		else if(chan==2) {temp_1d_4e[2]->Fill(ZZMass,weight); } 
	        else {temp_1d_2e2mu[2]->Fill(ZZMass,weight); }
	      } else if (j>9) {
		tnew[4]->Fill();
		temp_1d_4mu[4]->Fill(ZZMass,weight);
		temp_1d_4e[4]->Fill(ZZMass,weight);
	     	temp_1d_2e2mu[4]->Fill(ZZMass,weight);
	      } else {
		tnew[j]->Fill();
		if(chan==1) {temp_1d_4mu[j]->Fill(ZZMass,weight); }
		else if(chan==2) {temp_1d_4e[j]->Fill(ZZMass,weight); } 
	        else {temp_1d_2e2mu[j]->Fill(ZZMass,weight); }
	      }
	    }
	  }//entries loop  end
	}//file loop  end
	...
	hzx_ee = new TH1F("hzx_ee","hzx_ee",nBinsTempl,xmin,xmax); //full histogram
	...
	if (rebin==true){
		...
		for(int iq=1;iq<=newbins;iq++){
		  bincont1=temp_1d_4mu[2]->GetBinContent(iq);
		  temp_1d_4mu[2]->SetBinContent(iq,weightBSM[iq-1]*bincont1);
		  bincont2=temp_1d_4e[2]->GetBinContent(iq);
		  temp_1d_4e[2]->SetBinContent(iq,weightBSM[iq-1]*bincont2);                                                          
		  bincont3=temp_1d_2e2mu[2]->GetBinContent(iq);                                                                           
		  temp_1d_2e2mu[2]->SetBinContent(iq,weightBSM[iq-1]*bincont3);  
		}
	...
	sprintf(filename,"quad_MCyields_%d%s.txt",year,theExtra.c_str());
	ofstream yields(filename);
	yields << temp_1d_4mu[2]->Integral() << "\t" << temp_1d_4e[2]->Integral() << "\t" << temp_1d_2e2mu[2]->Integral() << endl;
	yields.close();
